    ...
    try {
        ...
        vehiculos.forEach(vehiculo => {
            ...
            tr.innerHTML = `
                ...
                <td>${vehiculo.marca} (${vehiculo.tipo_vehiculo || 'Sin Tipo'})</td>
                <td>${vehiculo.modelo}</td>
                <td>${vehiculo.capacidad_carga || vehiculo.capacidad} Kg</td> 
                <td>
...
const guardarVehiculo = async (e) => {
    e.preventDefault();
    const id = document.getElementById("vehiculoId").value.trim();
    const placa = document.getElementById("placa").value.trim();
    const marca = document.getElementById("marca").value.trim();
    const modelo = document.getElementById("modelo").value.trim();
    const capacidad_carga = document.getElementById("capacidad").value.trim();
    const tipo_vehiculo = document.getElementById("tipo_vehiculo").value;
    const estado = id ? document.getElementById("estado").value : "disponible";
    if (!placa || !marca || !modelo || !capacidad_carga || !tipo_vehiculo) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            document.getElementById("form-group-estado").classList.add("hidden"); 
            obtenerVehiculos(); 
        } else {
...
window.cargarVehiculoEdicion = (vehiculo) => {
    ...
    document.getElementById("capacidad").value = vehiculo.capacidad_carga || vehiculo.capacidad;
    if (vehiculo.tipo_vehiculo) {
        document.getElementById("tipo_vehiculo").value = vehiculo.tipo_vehiculo;
    }
    if (vehiculo.estado) {
        document.getElementById("estado").value = vehiculo.estado.toLowerCase();
    }
    document.getElementById("form-title").textContent = "Editar Vehículo 🚚";
    document.getElementById("btnCancelar").classList.remove("hidden");
    document.getElementById("form-group-estado").classList.remove("hidden"); 
};
